...
@task
def PostEmail():
	send_mail('converter', 'Ok', 'atknin.ii@physics.msu.ru', ['atknini@yahoo.com'], fail_silently=False)
...
#1------------полуширина-------------------------------------------------------------------------------------------------------
	...
	message = []
	message.append('полуширина - ')
	message.append(half_max)
	message.append('\n время расчета  - ')
	message.append(total_minutes)
	message.append(py_hwidth)
	send_mail('KDO', str(message) , 'atknin.ii@physics.msu.ru', ['atknini@yahoo.com'], fail_silently=False)
